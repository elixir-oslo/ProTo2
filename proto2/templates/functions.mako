...
<%def name="multichoice(name, opts, values, label = None, info = None, history = False)">
    <fieldset><legend>${label}</legend>
        ...
        %for key,value in opts.items():
            <label><input onchange="updateMultiChoice(this, '${name}', '${key}', ${str(history).lower()});reloadForm(form, this)" class="${name}" id="${name + '|' + key}" name="${name + '|' + key}" type="checkbox" value="${value if history else 'True'}" ${'checked=checked' if values and values[key] else 'checked=checked' if value and not values else ''}> ${unquote(str(value.split(':')[3])) if history else key}</label><br/>
        %endfor
...
<%def name="password(name, value='', label=None, reload=False, info = None)">
    <p><label>${label if label is not None else name} ${'<br>' if label else ''}
        <input type="password" name="${name}" value="${value}" autocomplete="nope" style="max-width:100%;width:100%" ${"onchange=\"reloadForm(form, this)\"" if reload else ''}>
    </label></p>
</%def>
